...
(function() {
    ...
    ns.removeContact = function() {
        var key = parseInt($(this).attr('data-key'));
        var results = retrieveFromStorage();
        $('#currentAction').html('Remove Contact');
        removed = results.filter(function(el) {
    return el.firstName !== results[key].firstName;
});

sessionStorage.setItem('contacts', JSON.stringify(removed));
ns.display();
